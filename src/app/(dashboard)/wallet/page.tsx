...
interface Transaction {
  id: string;
  type: "DEPOSIT" | "PLATFORM_FEE" | "REFUND" | "TASK_EARNING" | "TASK_PAYMENT" | "WITHDRAWAL";
  amount: number;
              ...
              <p className="text-2xl font-bold text-green-500">
                ...
                {(
                  walletData?.transactions
                    .filter((t) => {
                      return t.type === "TASK_EARNING" && t.status === "COMPLETED";
                    })
                    .reduce((sum, t) => {
                      return sum + t.amount;
                    }, 0) ?? 0
            ...
            <CardContent>
              <p className="text-2xl font-bold text-orange-500">
                ...
                {Math.abs(
                  walletData?.transactions
                    .filter((t) => {
                      return t.type === "TASK_PAYMENT" && t.status === "COMPLETED";
                    })
                    .reduce((sum, t) => {
                      return sum + t.amount;
                    }, 0) ?? 0,
            ...
            <CardContent>
              <p className="text-2xl font-bold text-gray-500">
                ...
                {Math.abs(
                  walletData?.transactions
                    .filter((t) => {
                      return t.type === "PLATFORM_FEE" && t.status === "COMPLETED";
                    })
                    .reduce((sum, t) => {
                      return sum + t.amount;
                    }, 0) ?? 0,
          ...
          <CardContent>
            {isConnected && address ? (
              ...
              <div className="space-y-4">
                ...
                <Button className="w-full sm:w-auto" onClick={handleConnectWallet}>
                  <IconWallet className="mr-2 size-4" />
                  Connect Coinbase Smart Wallet
                </Button>
                <p className="text-xs text-muted-foreground">
                  No crypto knowledge needed. Sign in with Face ID, fingerprint, or passkey.
                </p>
              </div>
            )}
          </CardContent>
          ...
          <CardContent>
            {walletData?.transactions && walletData.transactions.length > 0 ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">Add funds to start hiring agents or earning from tasks</p>
              </div>
            )}
          </CardContent>
        </Card>
